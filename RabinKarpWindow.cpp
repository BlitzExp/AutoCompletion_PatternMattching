...
// === Rabin–Karp + Binary Search: Longest Common Substring (contiguo) ===
string rabinKarpLCS(const string& S1, const string& S2) {
    if (S1.empty() || S2.empty()) return "";

    // Trabaja con A como la más corta (opcional, ayuda en tiempo/memoria)
    ...
    const uint64_t BASE = 256ULL; // trata cada char como byte
    ...
    auto hasCommonLen = [&](int L, int& outA, int& outB) -> bool {
        ...
        // Combina dos hashes en una sola llave 64-bit
        auto pack = [](uint64_t h1, uint64_t h2) -> uint64_t {
            return (h1 << 32) ^ h2;
        };

        // Indexa todas las ventanas de B de longitud L
        unordered_map<uint64_t, vector<int>> idx;
        idx.reserve((size_t)max(1, m - L + 1));
        ...
        for (int j = 1; j + L <= m; ++j) {
            unsigned char left = (unsigned char)B[j - 1];
            unsigned char right = (unsigned char)B[j + L - 1];
            hb1 = ((hb1 + MOD1 - (left * p1) % MOD1) * BASE + right) % MOD1;
            hb2 = ((hb2 + MOD2 - (left * p2) % MOD2) * BASE + right) % MOD2;
            idx[pack(hb1, hb2)].push_back(j);
        }

        // Recorre ventanas de A y busca coincidencias por hash; verifica con compare
        ...
        auto it0 = idx.find(pack(ha1, ha2));
        if (it0 != idx.end()) {
            for (int posB : it0->second) {
                if (A.compare(0, L, B, posB, L) == 0) { outA = 0; outB = posB; return true; }
            }
        }
        for (int i = 1; i + L <= n; ++i) {
            unsigned char left = (unsigned char)A[i - 1];
            unsigned char right = (unsigned char)A[i + L - 1];
    ...
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        int iA = 0, iB = 0;
        if (hasCommonLen(mid, iA, iB)) {
            bestLen = mid; bestA = iA; bestB = iB;
            lo = mid + 1;  // intenta más largo
        } else {
            hi = mid - 1;  // reduce
        }
    }

    // Devuelve el substring desde el string corto que usamos como A
    if (bestLen == 0) return "";
    return (S1.size() <= S2.size() ? S1.substr(bestA, bestLen)
                                   : S2.substr(bestA, bestLen));
}
...
int main() {
    ...
    double pct = denom ? (100.0 * best.size() / (double)denom) : 0.0;
